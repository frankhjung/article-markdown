...
`2 ∣ 3`. Reverting `h_2_dvd_3` turns the goal from `False` into
`2 ∣ 3 → False`, and [decide][decide] closes it by computation because
divisibility on concrete naturals is decidable, and `2 ∣ 3`.
...
Feeding `h_right_even` into `h_right_odd` is just Lean's formal way of saying:
"Look, I have a proof that `3^b` is even, and a proof that it can't be even.
This is a paradox, so the original assumption is False."
