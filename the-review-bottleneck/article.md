...
![Banner: The Review Bottleneck](images/banner.jpg)
...
left out entirely: sometimes bugs survive not because testing failed to catch
them, but because nobody was ever going to fix them.

That comment, and the conversation it started, turned into more than a reply. It
was the second half of the argument.

## The Bugs We Choose Not to Fix
...
crowdsourced testing whether they like it or not.

This is a different failure mode from the ones covered last time. Optus and
Telstra were systems that broke despite everyone's best efforts to keep them
running. Here, software remains broken simply because there is no incentive to
fix it. Testing did not fail here. Nobody asked it to succeed. That is the
point.

## From Shipped Complete to Good Enough

The reader also raised a historical point worth considering: this was not always
how software worked. A game used to ship as a finished object — the disc was the
product, in full. Now a game ships at 95 per cent complete, with the remaining 5
per cent arriving as a day-one patch, because the internet made "patch it later"
technically possible for the first time.
...
sense in a world where deferring is an option at all. Before the internet, it
was not.

## When Reviewers Cannot Keep Up
...
May 2026, over 80 per cent of the code merged into Anthropic's own codebase was
written by Claude, not by a human engineer. The company states plainly that once
AI and human code quality reach parity, human review — not generation — becomes
the bottleneck on how fast the whole pipeline can move.
...
correctness structural rather than hoping a human had time to look.

## The Mathematical Parallel

Terence Tao's [recent essay][tao_paper] describes the same shift happening in
mathematical proof. As AI generates proofs faster than the community can verify
or explain them, Tao argues that a proof is not finished until a human can
explain it. He also uses Lean 4 himself, for exactly this reason: while
...
Put these together and the shape of the problem changes. The reader's point
still stands: some bugs ship because nobody is incentivised to fix them, and no
amount of formal verification changes an organisation's growth targets. AI
acceleration compounds the problem in a different way. Even organisations with
every incentive to catch bugs are running out of human review capacity.
